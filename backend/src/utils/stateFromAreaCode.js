const { areaCodeFromE164 } = require('./phoneNormalize');

// Common NANP area codes by state. Not exhaustive (overlay/new codes are
// added by NANPA over time) — used as a best-effort fallback when a lead's
// state isn't provided, and in phase 2 for local-presence caller ID.
...
const STATE_BY_AREA_CODE = Object.entries(AREA_CODES_BY_STATE).reduce((map, [state, codes]) => {
  for (const code of codes) map[code] = state;
  return map;
}, {});

/** Looks up the US state for a phone number's area code. Returns null if unknown. */
function stateFromAreaCode(phone) {
  const areaCode = phone && phone.length === 3 ? phone : areaCodeFromE164(phone);
  if (!areaCode) return null;
  return STATE_BY_AREA_CODE[areaCode] || null;
}

module.exports = { stateFromAreaCode, AREA_CODES_BY_STATE, STATE_BY_AREA_CODE };
